...
export async function resolveEntity(label: string): Promise<EntityResolveResponse> {
  return fetchKoi<EntityResolveResponse>(`/entity/resolve?label=${encodeURIComponent(label)}`);
}

export async function getNeighborhood(label: string): Promise<NeighborhoodResponse> {
  return fetchKoi<NeighborhoodResponse>(`/entity/neighborhood?label=${encodeURIComponent(label)}`);
}
